...
var uiImageColorPicker = function(){
	...
	var _createImageColorPicker = function(widget) {
		...
		widget.$canvas = $('<canvas class="apparence_color_img"></canvas>');
  ...
  var lookupColor = function( imageData, point) {
	...
	var updateCurrentColor = function(widget, red, green, blue) {
		...
		c.fillRect (canvasWidth - 30, canvasHeight - 30, 30, 30);
	}
	...
	var updateSelectedColor = function(widget, red, green, blue) {
		...
		// draw current Color
		c.fillStyle = "rgb(" + red + "," + green + "," + blue + ")";
		c.fillRect (canvasWidth - 32, canvasHeight - 32, 0, 0);

		// draw border
		c.lineWidth = "3"
		c.lineJoin = "round";
		c.strokeRect (canvasWidth - 32, canvasHeight - 32, 0, 0);
