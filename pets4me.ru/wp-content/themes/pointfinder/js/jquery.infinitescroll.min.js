...
var self=this;opts.loading.start=opts.loading.start||function(){$(opts.navSelector).hide();opts.loading.msg.appendTo(opts.loading.selector).show(opts.loading.speed,$.proxy(function(){this.beginAjax(opts);},self));if(typeof options.loading.startCallback==="function"){$(""+opts.loading.selector2+" .pointfinder-infinite-scroll-loadbutton-num").text((opts.state.currPage+1));if(opts.state.currPage==opts.maxPage){$(""+opts.loading.selector2+" .pointfinder-infinite-scroll-loadbutton").remove();}
options.loading.startCallback.call();}};opts.loading.finished=opts.loading.finished||function(){opts.loading.msg.fadeOut(opts.loading.speed);};opts.callback=function(instance,data,url){if(!!opts.behavior&&instance['_callback_'+opts.behavior]!==undefined){instance['_callback_'+opts.behavior].call($(opts.contentSelector)[0],data,url);}
...
opts.state.isDone=true;opts.state.currPage=1;opts.state.isPaused=false;$("#infscr-loading").remove();this._binding('unbind');},_loadcallback:function infscr_loadcallback(box,data,url){var opts=this.options,callback=this.options.callback,result=(opts.state.isDone)?'done':(!opts.appendCallback)?'no-append':'append',frag;if(!!opts.behavior&&this['_loadcallback_'+opts.behavior]!==undefined){this['_loadcallback_'+opts.behavior].call(this,box,data);return;}
