...
{		
		...
		global $wp;
		if (isset($wp->request)) {
			$current_url = home_url(add_query_arg(array(),esc_url($wp->request)));
		}else{
		...
		wp_register_script('theme-google-api', 'https://maps.googleapis.com/maps/api/js?v=3.24&libraries=places&language='.$maplanguage.$setup5_map_key, array('jquery'), '',true); 
